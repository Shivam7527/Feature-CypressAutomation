...
productName= element.text();
   ...
   const path = Cypress.config("fileServerFolder");
    cy.readFile((path) + "\\cypress\\downloads\\order-invoice_shiva7628.csv").then(async (text) =>{

      const csv = await  neatCSV(text);
      console.log(csv);
     expect(csv[0]['Product Name']).to.eq(productName);
     console.log(csv[0]['Invoice Number']);
     expect(csv[0]['Product Price']).to.eq('31500');
     expect(csv[0]['Invoice Number']).to.eq(invoiceNumber);
